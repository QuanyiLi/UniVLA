...
# === 8-DoF Action Decoder for Panda (7 arm joints + 1 gripper) ===
ACTION_DIM = 8  # 7 arm joint targets + 1 gripper command

class ActionDecoder(torch.nn.Module):
    def __init__(self, window_size=20, hidden_dim=512):
        super().__init__()
...
class Wrapped_Model(torch.nn.Module):
    def __init__(self, vla, freeze_vla=False, window_size=20):
        super().__init__()
        self.vla = vla
    ...
    def action_decoder_forward(self, batch, vla_output):
        ...
        pred_action = self.action_decoder(latent_action_tokens, visual_embed).reshape(-1, self.window_size, ACTION_DIM)
...
class FinetuneConfig:
    # fmt: off
    vla_path: str = "qwbu/univla-7b"                               # Pretrained UniVLA path (HF hub or local)
    lam_path: str = "latent_action_model/logs/task_centric_lam_stage2/epoch=0-step=200000.ckpt"
    # Directory Paths
    data_root_dir: Path = Path("/path/to/rlds_train")               # Path to RLDS dataset directory
    dataset_name: str = "wiser_panda"                               # Mixture name in mixtures.py
    ...
    shuffle_buffer_size: int = 120                                 # Dataloader shuffle buffer size (can reduce if OOM)
    ...
    window_size: int = 20                                           # Action chunk size (20 frames @ 20Hz = 1s)
